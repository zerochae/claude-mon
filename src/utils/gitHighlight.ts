...
export function highlightGitStatus(raw: string): string {
  return raw
    .split("\n")
    .map((line) => {
      if (/^\s*M\s/.test(line) || line.includes("modified:")) return span(C.hash, line);
      if (/^\s*A\s/.test(line) || line.includes("new file:")) return span(C.add, line);
      if (/^\s*D\s/.test(line) || line.includes("deleted:")) return span(C.del, line);
      if (/^\?\?\s/.test(line) || line.includes("Untracked")) return span(C.dim, line);
...
export function detectGitOutput(content: string, output: string): GitOutputType {
  ...
  if (output.startsWith("commit ") || /^[0-9a-f]{7,40}\s/.test(output)) return "log";
  if (output.startsWith("On branch ")) return "status";
  return null;
}

export function highlightGitOutput(content: string, output: string): string | null {
